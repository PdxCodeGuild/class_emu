# lab12_guess_the_number_V4.py
'''
Lab 12: Guess the Number
Let's play 'Guess the Number'. The computer will guess a random int between 1 and 10.
The user will then try to guess the number, and the program will tell them whether they're right or wrong.

Concepts Covered
random.randint
REPL: read-evaluate-print loop
input, print

** Version 4 (optional) **
Tell the user whether their current guess is closer than their last.
This can be done by maintaining a variable containing the last guess outside the loop, then comparing the last guess to the current guess, and check if it's closer.
Hint: you're interested in comparing the two absolute differences: abs(current_guess-target) and abs(last_guess-target).
'''
import random # imports the random module for random generation
print("Hello.  Let's play 'Guess the Number'.  The computer will choose a number between 0 and 10.  Can you guess it? ") # prints the statement
computer_number = random.randint(0, 10) # defines the variable computer_number and generates a random number between 0 and 10
# print(computer_number) # printing the computer number for testing
user_tries = 0 # defines the variable user_tries and gives it a value of 0
first_guess = True
while True: # starts a while loop
    user_guess = int(input("What do you think the number between 0 and 10 will be? ")) # defines the variable user_guess and prompts user for input and converts that input to an integer
    user_tries += 1 # user_tries increments by 1 for every iteration of the while loop
    if user_guess == computer_number: # if statment inside the while loop to determine if user_guess equals computer_number
        print(f"You are Correct! The answer was {computer_number}. It took you {user_tries} tries to get it right.") # if the above statement is true, this code runs and F string makes the code inside curly brackets run as code {}
        break # if the above if statement is true, will break out of the loop after the print statement runs
    else: # if the above if statment is false, the below code runs
        print("Sorry, that is incorrect.") # prints this statement if the above if statement is false
    if first_guess == True: # establishes that if the first_guess is equal to True, the below code will run
        old_guess = user_guess
        first_guess = False # if the above is true, prints this statement
    else: # if the above if statement is false, the following code will run
        closeness_to_computer_old = abs(old_guess - computer_number) # defines variable and compares the absolute value between the old guess and computer number
        closeness_to_computer_new = abs(user_guess - computer_number) # defines variable and compares the absolute value between the new guess and computer number
        if closeness_to_computer_new < closeness_to_computer_old: # if statement to determine if the absolute value of the new guess is closer than the old guess
            print("You were closer") # if the above is true, this will print
        else: # if the above if statement is false, the following code will run
            print("You were not closer than last guess") # prints this statement if the above if statement is false
        old_guess = user_guess # makes the new guess the old guess for next iteration
